...
#    pytest-timeout backs the suite's @pytest.mark.timeout marker, dill backs its pickle tests, and
#    hypothesis backs the property layer (tests/v2/test_properties.py, imported at module level and
#    collected under testpaths -- without it that file's collection errors out the whole suite).
PKGS=(atheris pytest pytest-timeout dill hypothesis)
need_download=0
...
#    with no reinstall.
if "$PY" -c "import os,glob,sys; sys.exit(0 if (glob.glob(os.path.join('$SITE','atheris*')) and glob.glob(os.path.join('$SITE','pytest*')) and glob.glob(os.path.join('$SITE','pytest_timeout*')) and glob.glob(os.path.join('$SITE','dill*')) and glob.glob(os.path.join('$SITE','hypothesis*'))) else 1)"; then
...
PYTHONPATH="$PYRUN" "$PY" -c 'import atheris, nameparser, pytest, hypothesis; print("imports OK: nameparser", nameparser.__version__)'
